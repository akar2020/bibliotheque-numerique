...
const Auth = {
  ...
  redirectToLogin() {
    ...
    if (!current.endsWith('login.html') && !current.endsWith('forgot-password.html') && !current.endsWith('reset-password.html')) {
  ...
  // --- Protection de page (à appeler en haut de index.html) ---
  requireAuth() {
    if (!this.isAuthenticated()) {
      const refreshToken = this.getRefreshToken();
      if (refreshToken) {
        // Tenter le refresh silencieux
        this.refreshAccessToken().then(ok => {
